    ...
    void Model::loadRAM(){
        ...
        process_node = [this,&process_node, scene, &mes_num](const aiNode* node){
            for(unsigned int m = 0; m < node->mNumMeshes; m++){
                Mesh& mr = meshes_[mes_num];
                mes_num += 1;
                aiMesh *mesh = scene->mMeshes[node->mMeshes[m]]; 

                // Making vectors
                mr.vertices.resize(mesh->mNumVertices);
                unsigned int ind_num = 0;
                for(unsigned int f = 0; f < mesh->mNumFaces; f++){
                    ind_num += mesh->mFaces[f].mNumIndices;
                }
                mr.indices.resize(ind_num);

                // Filling vectors
                for(unsigned int v = 0; v < mesh->mNumVertices; v++){
                    Vertex& vtx = mr.vertices[v];
                ...
                for(unsigned int f = 0; f < mesh->mNumFaces; f++){
                    for(unsigned int i = 0; i < mesh->mFaces[f].mNumIndices; i++){
                        mr.indices[ind_num] = mesh->mFaces[f].mIndices[i];
                        ind_num += 1;
                    }
                }
            }
